...
# Agrupar os dados nas classes e obter as frequências absolutas
frequencias <- hist(dados, breaks = classes, plot = FALSE)$counts
frequencias
...
frequencias_esperadas <- sapply(2:length(classes), function(i) {
  n * (F_triangular(classes[i], a, b) - F_triangular(classes[i-1], a, b))
})

# Realizar o teste de qui-quadrado
chi_squared_test <- chisq.test(frequencias, p = frequencias_esperadas / n)
chi_squared_test

# Valor-p
valor_p <- chi_squared_test$p.value
valor_p
